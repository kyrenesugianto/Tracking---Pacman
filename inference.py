...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Define observations
    obs_distances = X_RANGE + Y_RANGE + MAX_NOISE - 1
    obs_all = list(range(0, obs_distances))

    variableDomainsDict[OBS0] = obs_all
    variableDomainsDict[OBS1] = obs_all
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # Initialize factors with evidence
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        for variable in eliminationOrder:
            # Join factors that contain the variable
            factorsToJoin, factorToEliminate = joinFactorsByVariable(factors, variable)
            ...
            if len(factorToEliminate.unconditionedVariables()) > 1:
                ...
                temp_factor = eliminate(factorToEliminate, variable)
                factorsToJoin.append(temp_factor)

            # Update factors for the next iteration
            factors = factorsToJoin
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        if total_value == 0:
            raise ValueError("Total value of the distribution is zero")
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            # Special case: Ghost is in jail, and the observation is None
            return 1 if noisyDistance is None else 0

        trueDistance = manhattanDistance(pacmanPosition, ghostPosition)

        if noisyDistance is None:
            # Special case: Ghost is not in jail, and the observation is None
            return 0

        # Use the provided function to get the observation probability
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldGhostPosition in allGhostPositions:
            newGhostPositionDist = self.getPositionDistribution(gameState, oldGhostPosition)
            for newGhostPosition, prob in newGhostPositionDist.items():
                newBeliefs[newGhostPosition] += beliefs[oldGhostPosition] * prob
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            ...
            discreteDistribution[particle] += observationProb
        ...
        if discreteDistribution.total() == 0:
            ...
            # Resample particles
            self.particles = [discreteDistribution.sample() for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        # Resample particles
        self.particles = [newBeliefs.sample() for i in range(self.numParticles)]
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if ghostDistance is not None:
                    probability *= self.getObservationProb(ghostDistance, pacmanPosition, particle[i], jailPosition)
                else:
                    # When ghost_distance is None, set likelihood to 0 and update the particle
                    probability = 0
                    particle = list(particle)
                    particle[i] = jailPosition
                    particle = tuple(particle)

            newDistribution[particle] += probability
