...
def joinFactors(factors: List[Factor]):
    ...
    # Combine unconditioned variables
    unconditioned = list(set().union(*(factor.unconditionedVariables() for factor in factors)))

    # Combine conditioned variables
    conditioned = list(set().union(*(factor.conditionedVariables() for factor in factors)))
    conditioned = [var for var in conditioned if var not in unconditioned]

    # Get the variable domains dictionary from the first factor
    variableDomainsDict = next(iter(factors)).variableDomainsDict()

    # Create a new factor
    newFactor = Factor(unconditioned, conditioned, variableDomainsDict)

    # Compute the product of corresponding rows of the input factors
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        new_factor = Factor(unconditioned_vars, factor.conditionedVariables(), factor.variableDomainsDict())
        ...
        for assignment in assignments:
            ...
            for value in factor.variableDomainsDict()[eliminationVariable]:
                ...
                prob_sum += factor.getProbability(new_assignment)
